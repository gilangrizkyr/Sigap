...
<head>
    ...
    <title><?= $page_title ?? 'DPMPTSP — Sigap' ?></title>
    <!-- Base styling -->
...
<body>
    ...
    <div style="position: relative; background: linear-gradient(135deg, #b71c1c 0%, #7f0000 100%); padding: 0 0 70px; color: #ffffff; box-shadow: inset 0 -10px 20px rgba(0,0,0,0.05); overflow: hidden;">
        ...
        $is_home_active = ($current_path === 'dpmptsp' || $current_path === 'pengaduan/dpmptsp');
        $is_tracking_active = (strpos($current_path, 'dpmptsp/tracking') !== false);
        $is_faq_active = (strpos($current_path, 'dpmptsp/faq') !== false);
        $is_about_active = (strpos($current_path, 'dpmptsp/about') !== false);
        ?>
        <div class="container" style="position: relative; z-index: 10;">
            <nav class="navbar">
                <a href="<?= base_url('dpmptsp') ?>" class="nav-logo">
                ...
                <ul class="nav-links" id="navLinks">
                    <li><a href="<?= base_url('dpmptsp') ?>" class="nav-link <?= $is_home_active ? 'active' : '' ?>">Home</a></li>
                    <li><a href="<?= base_url('dpmptsp/tracking') ?>" class="nav-link <?= $is_tracking_active ? 'active' : '' ?>">Lacak Aduan</a></li>
                    <li><a href="<?= base_url('dpmptsp/faq') ?>" class="nav-link <?= $is_faq_active ? 'active' : '' ?>">FAQ</a></li>
                    <li><a href="<?= base_url('dpmptsp/about') ?>" class="nav-link <?= $is_about_active ? 'active' : '' ?>">Tentang</a></li>
                </ul>
            </nav>
        </div>
