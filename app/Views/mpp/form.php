...
<div style="text-align: center; max-width: 800px; margin: 0 auto 20px;">
    ...
    <p style="color: rgba(255,255,255,0.95); font-size: 15px; margin-bottom: 0; line-height: 1.6; max-width: 720px; margin-left: auto; margin-right: auto;">
        Sampaikan pengaduan, aspirasi, atau saran Anda secara langsung terkait unit pelayanan di Mal Pelayanan Publik (MPP) Kabupaten Tanah Bumbu secara cepat, transparan, dan terintegrasi.
    </p>
</div>
...
<div class="card" style="padding: 32px; color: #111827; max-width: 800px; margin: 0 auto 30px; box-shadow: 0 20px 40px rgba(0,0,0,0.15);">
    <form action="<?= base_url('mpp/submit') ?>" method="POST" enctype="multipart/form-data" id="complaintForm">
        <?= csrf_field() ?>
        <input type="hidden" name="complaint_type" id="complaint_type" value="Pengaduan">

        <!-- Unit Layanan MPP (khusus MPP) -->
        <div class="form-group">
            <label class="form-label" for="service_unit_id" style="color: #374151;">
                Unit Layanan MPP <span style="color: var(--danger)">*</span>
            </label>
            <select name="service_unit_id" id="service_unit_id" class="form-control" required>
                <option value="" disabled selected>Pilih Unit Layanan MPP...</option>
                <?php foreach ($service_units as $unit): ?>
                    <option value="<?= $unit['id'] ?>" <?= old('service_unit_id') == $unit['id'] ? 'selected' : '' ?>><?= esc($unit['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
...
echo view('layouts/mpp', [
    'page_title'    => 'Form Pengaduan MPP — Sigap',
